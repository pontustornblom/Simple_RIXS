...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        complete_file_location= create_complete_file_location_for_treated_data.create_complete_file_location_for_treated_data(self.parameters["input_file_project_folder"], self.parameters["input_complete_file_name_array"][0])
        ...
        complete_file_name= self.parameters["input_complete_file_name_array"][0]
        #self.parameters= self.treated_parameters
        #self.parameters["input_complete_file_name_array"][0]= complete_file_name
        
        #self.parameters["output_file_element"] = self.treated_parameters["output_file_element"]
        #self.parameters["output_file_edge"] = self.treated_parameters["output_file_edge"]
        #self.parameters["output_file_sample_name"] = self.treated_parameters["output_file_sample_name"]
        #self.parameters["output_file_additional_comment"] = self.treated_parameters["output_file_additional_comment"]
        ...
        self.setWindowTitle("Simple RIXS view treated plot script")
        self.show()
        ...
        if self.validate_input(item, key):
            ...
            if int(item_text) != 0:
                for pfy_region in range(int(item_text)):
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+1,self.create_gui_item("pfy_region_name_array_"+ str(pfy_region), "Name of PFY region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+2,self.create_gui_item("pfy_incoming_energy_start_array_"+ str(pfy_region), "Lowest incoming energy of PFY region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+3,self.create_gui_item("pfy_incoming_energy_end_array_"+ str(pfy_region), "Highest incoming energy of PFY region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+4,self.create_gui_item("pfy_energy_loss_start_array_"+ str(pfy_region), "Lowest emission energy of PFY region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+5,self.create_gui_item("pfy_energy_loss_end_array_"+ str(pfy_region), "Highest emission energy of PFY region "+ str(pfy_region)+ ":                     ", "q_line_edit", [""]))
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        x_values_array= x_values_dataframe.iloc[:, 0].values
        ...
        if parameters["plot_waterfall_instead_of_heat_map"]== True:
            if parameters["plot_invert_y_axis"]== False:
                for spectra in range(len(incoming_energy_array)):
                    ax.plot(x_values_array, array_of_intensity_arrays[spectra] + float(parameters["plot_y_distance_between_plots"]) * spectra)
                ...
                for spectra in range(len(incoming_energy_array)):
                    ax.plot(x_values_array, reversed_array_of_intensity_arrays[spectra] + float(parameters["plot_y_distance_between_plots"]) * spectra)
                    if parameters["plot_display_incoming_energy_by_lines"]:
            ...
            im = ax.pcolormesh(x_values_array, incoming_energy_array, array_of_intensity_arrays, cmap=colormap)
        ...
        #im = axs[0].imshow(array_of_intensity_arrays, cmap='viridis', origin='lower', extent=[x_values[0], x_values[-1], incoming_energy_array[0] - ((incoming_energy_array[1]-incoming_energy_array[0])/2), incoming_energy_array[-1] + ((incoming_energy_array[-1]-incoming_energy_array[-2])/2)] , aspect='auto')
        #im = axs.pcolormesh(x_values, incoming_energy_array, array_of_intensity_arrays, cmap=colormap)
        ...
        if parameters["is_energy_window_used_array"][array_index]:
            ax.set_xlim(float(parameters["plot_energy_loss_min_array"][array_index]), float(parameters["plot_energy_loss_max_array"][array_index]))
            ax.set_ylim(float(parameters["plot_incoming_energy_min_array"][array_index]), float(parameters["plot_incoming_energy_max_array"][array_index]))
