...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.vbox.addLayout(self.create_gui_item("number_of_pfy_regions", "How many areas would you like to intergate over? ", "q_line_edit", [""]))
        ...
        self.setWindowTitle("Simple RIXS select integration region")
        self.show()
            ...
            if self.is_first_time_creating_pfy_regions== False:
                ...
                if old_number_of_pfy_regions>0:
                    for pfy_region in range(old_number_of_pfy_regions):
                        self.remove_item(self.vbox.indexOf(hbox)+5*pfy_region+1)
                        self.remove_item(self.vbox.indexOf(hbox)+5*pfy_region+2)
                        self.remove_item(self.vbox.indexOf(hbox)+5*pfy_region+3)
                        self.remove_item(self.vbox.indexOf(hbox)+5*pfy_region+4)
                        self.remove_item(self.vbox.indexOf(hbox)+5*pfy_region+5)
            if int(item_text) != 0:
                for pfy_region in range(int(item_text)):
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+1,self.create_gui_item("pfy_region_name_array_"+ str(pfy_region), "Name of integration region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+2,self.create_gui_item("pfy_energy_loss_start_array_"+ str(pfy_region), "Lowest emission energy of integration region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+3,self.create_gui_item("pfy_energy_loss_end_array_"+ str(pfy_region), "Highest emission energy of integration region "+ str(pfy_region)+ ":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+4,self.create_gui_item("pfy_incoming_energy_start_array_"+ str(pfy_region), "Lowest excitation energy of integration region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+5*pfy_region+5,self.create_gui_item("pfy_incoming_energy_end_array_"+ str(pfy_region), "Highest excitation energy of integration region "+ str(pfy_region)+ ":                     ", "q_line_edit", [""]))
    ...
    def get_arrays_of_pfy_intensity_and_incoming_energy_arrays(self, parameters):
        ...
        print("Right now it is updated so each inteisty spectra has its own x_array, however no interpolation has been done as I dont think it is nessesary in this case. I could be wrong tho.")

        array_of_integration_values = []
        for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
            ...
            array_of_x_values_arrays, incoming_energy_array, array_of_intenisty_arrays = get_treated_rixs_data_script.get_treated_rixs_data(complete_file_location)
            
            if parameters["plot_outgoing_energy_instead_of_energy_loss"]:
                if self.nested_array_contains_negative_floats(array_of_x_values_arrays):
                    for spectra_index in range(len(array_of_x_values_arrays)):
                        array_of_x_values_arrays[spectra_index]= array_of_x_values_arrays[spectra_index] + incoming_energy_array[spectra_index]
                #ax.set_xlabel("Emission energy [eV]", fontsize= parameters["plot_x_axis_text_size"])
            else:
                if self.nested_array_contains_negative_floats(array_of_x_values_arrays) == False:
                    for spectra_index in range(len(array_of_x_values_arrays)):
                        array_of_x_values_arrays[spectra_index]= array_of_x_values_arrays[spectra_index] - incoming_energy_array[spectra_index]
            ...
            for pfy_region in range(int(self.parameters["number_of_pfy_regions"])):
                array_of_area_per_spectra_index= []
                array_of_incoming_energy_arrays.append(incoming_energy_array)

                excitation_energy_index_start= np.abs(incoming_energy_array - float(parameters["pfy_incoming_energy_start_array"][pfy_region])).argmin()
                excitation_energy_index_end= np.abs(incoming_energy_array - float(parameters["pfy_incoming_energy_end_array"][pfy_region])).argmin()

                if excitation_energy_index_start == excitation_energy_index_end:
                    energy_loss_index_start= np.abs(array_of_x_values_arrays[excitation_energy_index_start] - float(parameters["pfy_energy_loss_start_array"][pfy_region])).argmin()
                    energy_loss_index_end= np.abs(array_of_x_values_arrays[excitation_energy_index_start] - float(parameters["pfy_energy_loss_end_array"][pfy_region])).argmin()
                    intensity_array= array_of_intenisty_arrays[excitation_energy_index_start][energy_loss_index_start : energy_loss_index_end + 1]
                    total_area= 0
                    for datapoint in range(len(intensity_array) - 1):
                        total_area+= ((intensity_array[datapoint] + intensity_array[datapoint + 1]) / 2) * (array_of_x_values_arrays[excitation_energy_index_start][datapoint + 1] - array_of_x_values_arrays[excitation_energy_index_start][datapoint])
                    array_of_integration_values.append(total_area)
                else:
                    
                    for spectra_index in range(excitation_energy_index_start, excitation_energy_index_end + 1):
                        ...
                        total_area= 0
                        for datapoint in range(len(intensity_array) - 1):
                            total_area+= ((intensity_array[datapoint] + intensity_array[datapoint + 1]) / 2) * (array_of_x_values_arrays[spectra_index][datapoint + 1] - array_of_x_values_arrays[spectra_index][datapoint])
                        array_of_area_per_spectra_index.append(total_area)
                    
                    total_area= 0
                    for index, spectra_index in enumerate(range(excitation_energy_index_start, excitation_energy_index_end)):
                        total_area+= ((array_of_area_per_spectra_index[index] + array_of_area_per_spectra_index[index + 1]) / 2) * (incoming_energy_array[spectra_index + 1] - incoming_energy_array[spectra_index])
                    array_of_integration_values.append(total_area)
                #array_of_integration_values.append(total_area)

        return array_of_integration_values
    
    def rearange_order_of_integration_values(self, parameters, array_of_integration_values):
        number_of_integration_regions = int(parameters["number_of_pfy_regions"])
        correct_array_of_integration_values = []
        for integration_region_index in range(int(parameters["number_of_pfy_regions"])):
            correct_array_of_integration_values.append([])

        for integration_value_index in range(len(array_of_integration_values)):
            correct_array_of_integration_values[integration_value_index % number_of_integration_regions].append(array_of_integration_values[integration_value_index])

        return correct_array_of_integration_values


    def print_integration_values(self, parameters):


        array_of_integration_values = self.get_arrays_of_pfy_intensity_and_incoming_energy_arrays(parameters)
        array_of_integration_values = self.rearange_order_of_integration_values(parameters, array_of_integration_values)

        for integration_region_index in range(int(parameters["number_of_pfy_regions"])):
            print("Integration region " + parameters["pfy_region_name_array"][integration_region_index] + ":")
            print(parameters["input_complete_file_name_array"][:int(self.parameters["input_number_of_complete_file_names"])])
            print(array_of_integration_values[integration_region_index])
    ...
    def save_and_continue(self):
        ...
        self.print_integration_values(self.parameters)
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        x_values_array, y_values_array, array_of_intensity_arrays = get_treated_rixs_data_script.get_treated_rixs_data(complete_file_location)
        if parameters["plot_outgoing_energy_instead_of_energy_loss"]:
            if self.nested_array_contains_negative_floats(x_values_array):
                for spectra_index in range(len(x_values_array)):
                    x_values_array[spectra_index]= x_values_array[spectra_index] + y_values_array[spectra_index]
            #ax.set_xlabel("Emission energy [eV]", fontsize= parameters["plot_x_axis_text_size"])
        else:
            if self.nested_array_contains_negative_floats(x_values_array) == False:
                for spectra_index in range(len(x_values_array)):
                    x_values_array[spectra_index]= x_values_array[spectra_index] - y_values_array[spectra_index]
            #ax.set_xlabel("Energy loss [eV]", fontsize= parameters["plot_x_axis_text_size"])
        ...
        self.figure_to_save, axs = plt.subplots(1)
        #intensity_max=100
        #intensity_min=0
        ...
        axs.set_xlabel(x_values_header)
        axs.set_ylabel("Excitation energy [eV]")
        ...
        for pfy_region in range(int(parameters["number_of_pfy_regions"])):
            ...
            try:
                ...
                incoming_energy_start = float(parameters["pfy_incoming_energy_start_array"][pfy_region])
                energy_loss_end = float(parameters["pfy_energy_loss_end_array"][pfy_region])
                incoming_energy_end = float(parameters["pfy_incoming_energy_end_array"][pfy_region])
