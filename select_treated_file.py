...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.vbox.addLayout(self.create_gui_item("input_complete_file_name_array_0", "Input complete file name of the treated data: \n(Can be either figure or data name)\n(You can drop the file into the text box)", "q_line_edit", [""]))
        self.vbox.addLayout(self.create_gui_item("open treated file", "Would you like to open the treated data file? ", "q_push_button",  [""]))
        if self.parameters["plot_type"] == "Plot treated spectra":
            self.vbox.addLayout(self.create_gui_item("txt_intensity_data_column", "Which spectra would you like to plot?  \nFor example: Input 3 to plot Spectra_3. \n(Just input 1 if it is XAS data) ", "q_line_edit", [""]))
        ...
        self.setWindowTitle("Simple RIXS Select treated file")
        self.show()
    ...
    def open_treated_file(self, item, item_text, key):
        self.parameters, complete_file_location = create_complete_file_location_view_roots_or_txt_script.create_complete_file_location_view_roots_or_txt(self.parameters, False, self.parameters["input_complete_file_name_array"][0])
