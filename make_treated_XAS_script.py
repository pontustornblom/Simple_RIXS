...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.setWindowTitle("Simple RIXS Make PFY spectra from several treated files")
        self.show()
    ...
    def create_dynamic_gui_item_for_pfy_inputs(self):
        ...
        for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
            for pfy_region in range(int(self.parameters["number_of_pfy_regions"])):
                ...
                if self.parameters["is_subtract_fitted_background_array"][array_index]:
                    self.vbox.addLayout(self.create_gui_item("background_fit_type_array_" + str(array_index), "What type of graph would you like to fit for this region? ", "q_combo_box", ["Linear", "ln(x)", "log(x)", "Gaussian"]))
    ...
    def save_treated_data(self):
        #for pfy_region in range(int(self.parameters["number_of_pfy_regions"])):
        figure_name= "PFY_spectra_comparison_waterfall"
        ...
        for spectra_index in range(int(self.parameters["number_of_pfy_regions"])):
            figure_name+="_" + str(round(float(self.parameters["pfy_energy_loss_start_array"][spectra_index])))
            figure_name+="_" + str(round(float(self.parameters["pfy_energy_loss_end_array"][spectra_index])))
        if self.parameters["is_energy_window_used_array"][0]:
        ...
        for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
            for pfy_region_index in range(int(self.parameters["number_of_pfy_regions"])):
                data_dictionary["Excitation energy [eV]" + '_' + self.parameters["plot_legend_names_array"][file_name_index] + '_' + self.parameters["pfy_region_name_array"][pfy_region_index]]= self.array_of_pfy_incoming_energy_arrays[array_index]
                data_dictionary["Intensity [a.u]" + "_" + self.parameters["plot_legend_names_array"][file_name_index] + '_' + self.parameters["pfy_region_name_array"][pfy_region_index]]= self.array_of_pfy_intensity_arrays[array_index]
                array_index+= 1
    ...
    def treat_intensity_data(self, parameters, array_of_pfy_intensity_arrays, array_of_pfy_incoming_energy_arrays):
        array_index = 0
        #for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
        #    for pfy_region in range(int(parameters["number_of_pfy_regions"])):
        incoming_energy_array = array_of_pfy_incoming_energy_arrays[array_index]
        y_values = array_of_pfy_intensity_arrays[array_index]
        ...
        if parameters["is_invert_the_plot_array"][array_index]== True:
            ...
            if parameters["is_subtract_fitted_background_array"][array_index]:
            ...
            elif parameters["is_normalize_all_to_zero_and_one"]:
        ...
        #array_index+= 1
        array_of_pfy_intensity_arrays = y_values

        return array_of_pfy_intensity_arrays
    ...
    def get_plot_color_and_linestyle_array(self, parameters):
        ...
        if parameters["plot_spectra_grouping_type"]== "No grouping":
            ...
            final_plot_color_array=[]
            final_linestyles_array=[]
            print(parameters["plot_file_name_index_to_change_color_palette_array"][ : int(parameters["plot_number_of_times_to_reset_color_palette"])])
            for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
                for pfy_region_index in range(int(self.parameters["number_of_pfy_regions"])):
    ...
    def get_intensity_and_incoming_energy_arrays(self, parameters):
        #This was made hastely for Varuns data
        is_single_data = False 
        data_row = parameters["txt_intensity_data_row"]
        data_column = parameters["txt_intensity_data_column"]
        is_iteratable_file_number = False
        iteratable_file_number = 0
        combine_several_columns = False
        intensity_array = get_single_spectrum_h5_or_txt_file_scripts.get_single_spectrum_txt_file(parameters, is_single_data, data_row, data_column, is_iteratable_file_number, iteratable_file_number, combine_several_columns)
        
        is_single_data = False 
        data_row = parameters["txt_incoming_energy_row_in_file"]
        data_column = parameters["txt_incoming_energy_column_in_file"]
        is_iteratable_file_number = False
        iteratable_file_number = 0
        combine_several_columns = False
        excitation_energy_array = get_single_spectrum_h5_or_txt_file_scripts.get_single_spectrum_txt_file(parameters, is_single_data, data_row, data_column, is_iteratable_file_number, iteratable_file_number, combine_several_columns)
        
        return intensity_array, excitation_energy_array
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        #This has been hastely made to treat Varuns data
        self.intensity_array, self.incoming_energy_array = self.get_intensity_and_incoming_energy_arrays(parameters)
        self.intensity_array = self.treat_intensity_data(parameters, self.intensity_array, self.incoming_energy_array)

        intensity_array_to_plot = np.array(self.intensity_array)
        incoming_energy_array_to_plot = np.array(self.incoming_energy_array)
        
        self.figure_to_save, ax = plt.subplots(1)
        self.figure_to_save.set_frameon(False)

        ax.plot(incoming_energy_array_to_plot, intensity_array_to_plot)
        ...
        #self.array_of_pfy_intensity_arrays = np.array(self.array_of_pfy_intensity_arrays)
        #self.array_of_pfy_incoming_energy_arrays = np.array(self.array_of_pfy_incoming_energy_arrays)
        
        array_of_intensity_arrays_to_plot = self.array_of_pfy_intensity_arrays
        array_of_x_values_arrays_to_plot = self.array_of_pfy_incoming_energy_arrays
        if parameters["plot_spectra_grouping_type"]== "No grouping":
            legend_array = []
            for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
                if int(self.parameters["number_of_pfy_regions"]) == 1:
                    ...
                    if parameters["plot_display_incoming_energy_by_lines"]:
                        #formatted_energy_loss_start = "{:.{}g}".format(float(parameters["pfy_energy_loss_start_array"][0]) , int(parameters["plot_incoming_energy_significant_numbers"]))
                        #formatted_energy_loss_end = "{:.{}g}".format(float(parameters["pfy_energy_loss_end_array"][0]) , int(parameters["plot_incoming_energy_significant_numbers"]))
                        ...
                        ax.text(float(parameters["plot_incoming_energy_x_offset"]) + float(array_of_x_values_arrays_to_plot[file_name_index][0]), float(parameters["plot_incoming_energy_y_offset"]) + float(parameters["plot_y_distance_between_plots"]) * file_name_index, parameters["pfy_region_name_array"][0], fontsize= float(parameters["plot_incoming_energy_text_size"]))
                else:
                    for pfy_region_index in range(int(self.parameters["number_of_pfy_regions"])):
                        legend_array.append(parameters["plot_legend_names_array"][file_name_index] + " " + parameters["pfy_region_name_array"][pfy_region_index])
            for array_index in range(int(parameters["input_number_of_complete_file_names"])*int(parameters["number_of_pfy_regions"])):
               ax.plot(array_of_x_values_arrays_to_plot[array_index], np.asarray(array_of_intensity_arrays_to_plot[array_index]) + float(parameters["plot_y_distance_between_plots"])*array_index, color=plot_color_array[array_index], linestyle= linestyles_array[array_index])
                ...
                for incoming_energy_index in range(int(self.parameters["number_of_pfy_regions"])):
                    ...
                    if parameters["is_hide_certain_spectra_array"][array_index] == False:
                        if parameters["plot_is_scale_intensity_of_spectra_array"][array_index] == True:
                            array_of_intensity_arrays_to_plot[array_index]= np.asarray(array_of_intensity_arrays_to_plot[array_index]) * float(parameters["plot_scaling_array"][array_index]) 
                            formatted_scaling_text = "{:.{}g}".format(float(parameters["plot_scaling_array"][array_index]), 8)
                            ax.text(float(parameters["plot_scaling_text_x_offset_array"][array_index]) + float(array_of_x_values_arrays_to_plot[array_index][0]), float(parameters["plot_scaling_text_y_offset_array"][array_index]) + float(parameters["plot_distance_between_groups_in_y"]) * incoming_energy_index + float(parameters["plot_y_distance_between_plots"]) * file_name_index, "x" + formatted_scaling_text, fontsize= float(parameters["plot_incoming_energy_text_size"]), color= plot_color_array[file_name_index])
                        
                        if parameters["is_gaussian_smooth_data_for_all_spectra"]:
                            array_of_intensity_arrays_to_plot[array_index]= gaussian_filter1d(array_of_intensity_arrays_to_plot[array_index], float(parameters["sigma_for_gaussian_smoothing_array"][0]))
                    ...
                    if file_name_index == 0:
                        if parameters["plot_display_incoming_energy_by_lines"]:
                            ...
                            #text = f'y={exact_energy_per_channel_slope:.8f}x+{exact_energy_per_channel_intercept:.2f}\nR={exact_energy_per_channel_r_value:.8f} \nStandard error={exact_energy_per_channel_std_err:.8f}'
                            ax.text(float(parameters["plot_incoming_energy_x_offset"]) + float(array_of_x_values_arrays_to_plot[array_index][0]), float(parameters["plot_incoming_energy_y_offset"]) + float(parameters["plot_distance_between_groups_in_y"]) * incoming_energy_index + float(parameters["plot_y_distance_between_plots"]) * file_name_index,  parameters["pfy_region_name_array"][file_name_index], fontsize= float(parameters["plot_incoming_energy_text_size"]))
