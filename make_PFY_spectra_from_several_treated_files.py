#make_PFY_spectra_from_several_treated_files
...
class MainWindow(QMainWindow):
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        #linestyles_array = ['-', '--', '-.', '-', '--', '-.']
        #plot_color_array = ['blue', 'red', 'green', 'blue', 'red', 'green', 'magenta', 'magenta', 'black']
        #linestyles_array = ['-', '-', '-', '-', '-', '-', '-', '-.', '-']
        #plot_color_array = ['blue', 'red', 'green', 'magenta', 'black']
        #linestyles_array = ['-', '-', '-', '-', '-']
                ...
                for incoming_energy_index in range(int(self.parameters["number_of_pfy_regions"])):
                    ...
                    if file_name_index == 0:
                        if parameters["plot_display_incoming_energy_by_lines"]:
                            ...
                            #text = f'y={exact_energy_per_channel_slope:.8f}x+{exact_energy_per_channel_intercept:.2f}\nR={exact_energy_per_channel_r_value:.8f} \nStandard error={exact_energy_per_channel_std_err:.8f}'
                            ax.text(float(parameters["plot_incoming_energy_x_offset"]) + float(array_of_x_values_arrays_to_plot[array_index][0]), float(parameters["plot_incoming_energy_y_offset"]) + float(parameters["plot_distance_between_groups_in_y"]) * incoming_energy_index + float(parameters["plot_y_distance_between_plots"]) * file_name_index,  parameters["pfy_region_name_array"][incoming_energy_index], fontsize= float(parameters["plot_incoming_energy_text_size"]))
        ...
        if parameters["is_display_legend"]:
            ...
            if False:
                ...
                plt.legend(handles=[black_solid, black_dashed, black_dashdot, FePO4_linestyle, LiFePO4_linestyle, FeP2O7_linestyle], fontsize= parameters["plot_legend_text_size"], loc="best")

            elif False:
                black_solid = mlines.Line2D([], [], color='blue', linestyle='-', label=r'Li$_3$PO$_4$')
                black_dashed = mlines.Line2D([], [], color='red', linestyle='-', label=r'Na$_3$PO$_4$')
                black_dashdot = mlines.Line2D([], [], color='green', linestyle='-', label=r'K$_3$PO$_4$')
                FePO4_linestyle = mlines.Line2D([], [], color='magenta', linestyle='-', label=r'Na$_4$P$_2$O$_7$')
                FeP2O7_linestyle = mlines.Line2D([], [], color='black', linestyle='-', label=r'K$_4$P$_2$O$_7$')

                plt.legend(handles=[black_solid, black_dashed, black_dashdot, FePO4_linestyle, FeP2O7_linestyle], fontsize= parameters["plot_legend_text_size"], loc=(0.05, 0.7))
