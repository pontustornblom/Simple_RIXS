#make_treated_veritas_RIXS_script
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        if self.parameters["is_input_file_names_manually"]:
            ...
            incoming_energy_array = self.get_incoming_energy_array_veritas(self.parameters, iteratable_file_number_array)
        ...
        self.setWindowTitle("Simple RIXS Make treated Veritas RIXS")
        self.show()
    ...
    def update_dictionary_array(self, key, array_index, item):
        ...
        elif isinstance(item, str):                      # <-- new branch
            self.parameters[key][array_index] = item
    ...
    def get_incoming_energy_array_veritas(self, parameters, iteratable_file_number_array):
        ...
        if parameters["input_file_format"]== "h5":
            for iteratable_number in iteratable_file_number_array:
                ...
                incoming_energy_array.append(incoming_energy)
        if len(incoming_energy_array) == 0:
            raise ValueError(
                "No incoming energies found. "
                "Check that 'input_file_format' is 'h5' (currently: '{}') "
                "and that the file number range ({} to {}) is valid.".format(
                    parameters.get("input_file_format", "not set"),
                    parameters.get("input_file_iteratable_file_number_start", "?"),
                    parameters.get("input_file_iteratable_file_number_end", "?"),
                )
            )
        if isinstance(incoming_energy_array[0], np.ndarray) or isinstance(incoming_energy_array[0], list):
            if len(incoming_energy_array[0]) >1: 
    ...
    def get_single_i0_spectrum_h5_veritas(self, parameters, iteratable_number):
        ...
        #The one below is for Veritas, the row below that is for species
    ...
    def get_single_i0_spectrum_h5_veritas_manual_file_name(self, parameters, iteratable_number):
        complete_file_location = os.path.join(parameters["input_file_project_folder"], parameters["input_file_raw_data_folder"], parameters["input_complete_file_name_array"][int(iteratable_number)])
        #The one below is for Veritas, the row below that is for species
        root_location= "entry" + str(iteratable_number) +"/measurement/aemexp2_ch1"
        #root_location= "entry" + str(iteratable_number) +"/measurement/aem_rixs_ch4"
        raw_data_list=[]
    ...
    def get_array_of_intensity_arrays_veritas_manual_file_names(self, parameters, iteratable_file_number_array, iteratable_file_number_array_i0):
        ...
        for spectra_index in range(len(iteratable_file_number_array)):
            ...
            #    i0_values = self.get_single_i0_spectrum_h5_veritas(parameters, 228)
            if parameters["is_normalization_to_i0"]:
                i0_values = self.get_single_i0_spectrum_h5_veritas_manual_file_name(parameters, iteratable_file_number_array_i0[spectra_index])
                i0_sum= np.sum(i0_values)
                y_values= y_values/i0_sum
            array_of_intensity_arrays[intensity_array_index]= y_values
            intensity_array_index+= 1
    ...
    def move_elastic_peak_center_to_correct_energy(self, array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays):
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            ...
            while condition:
                ...
                for channel in range(max(0, previous_elastic_peak_channel_center - channels_above_and_below_elastic_to_fit), min(n_channels_p1, previous_elastic_peak_channel_center + channels_above_and_below_elastic_to_fit + 1)):
                ...
                if abs(change_in_peak_channel_center) <= 0.5:
                    if self.parameters["degree_of_energy_per_channel_polynomial"] == "0":
                        ...
                        if spectra_index == 0:
                            if self.parameters["is_input_energy_per_channel_and_intercept_manually"] == False or self.parameters["is_x_values_available_in_file"] == False:
    ...
    def move_elastic_peak_center_to_correct_energy_with_gaussian(self, array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays):
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            try:
                ...
                #channel_of_elastic_peak_maximum = previous_elastic_peak_channel_center - channels_above_elastic_to_fit + np.abs(array_of_intensity_arrays[spectra_index][previous_elastic_peak_channel_center - channels_above_elastic_to_fit : previous_elastic_peak_channel_center + channels_above_elastic_to_fit + 1] - np.max(array_of_intensity_arrays[spectra_index][previous_elastic_peak_channel_center - channels_above_elastic_to_fit : previous_elastic_peak_channel_center + channels_above_elastic_to_fit + 1])).argmin()
                n_channels_gauss = len(array_of_x_value_arrays[spectra_index])
                half_channel_width_of_elastic_peak = np.abs(array_of_intensity_arrays[spectra_index][previous_elastic_peak_channel_center : ] - rough_half_elastic_peak_intenisty).argmin()
                if half_channel_width_of_elastic_peak <= 0:
                    half_channel_width_of_elastic_peak = 1
                channel_of_half_elastic_peak_width_above_elastic = min(previous_elastic_peak_channel_center + half_channel_width_of_elastic_peak, n_channels_gauss - 1)
                half_energy_width_of_elastic_peak = array_of_x_value_arrays[spectra_index][channel_of_half_elastic_peak_width_above_elastic] - array_of_x_value_arrays[spectra_index][previous_elastic_peak_channel_center]
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        if parameters["is_input_file_names_manually"] == False:
            ...
            array_of_x_value_arrays = []
            for file_name_index in range(int(parameters["input_complete_file_name_array"])):
        ...
        #polynomial_coefficients = np.flip(np.polyfit(energy_calibration_channel_position_array, energy_calibration_energy_position_array, int(parameters["degree_of_energy_per_channel_polynomial"])))
        is_x_values_energy_loss = any(number < 0 for number in array_of_x_value_arrays[0])
        if is_x_values_energy_loss:
            number_of_channels_to_fit_start = np.abs(array_of_x_value_arrays[0] - 0).argmin() #do this because we dont want to fit edgepoint data and check first that it is not energy loss
            number_of_channels_to_fit_end = np.abs(array_of_x_value_arrays[0] - (incoming_energy_array[-1] - incoming_energy_array[0])).argmin()
        else:
            number_of_channels_to_fit_start = np.abs(array_of_x_value_arrays[0] - incoming_energy_array[0]).argmin() #do this because we dont want to fit edgepoint data and check first that it is not energy loss
            number_of_channels_to_fit_end = np.abs(array_of_x_value_arrays[0] - incoming_energy_array[-1]).argmin()
        if parameters["is_x_values_available_in_file"]:
            polynomial_coefficients = np.flip(np.polyfit(np.arange(0, len(array_of_x_value_arrays[0])), array_of_x_value_arrays[0], int(parameters["degree_of_energy_per_channel_polynomial"])))

        else:
            polynomial_coefficients = np.flip(np.polyfit(np.arange(number_of_channels_to_fit_start, number_of_channels_to_fit_end), array_of_x_value_arrays[0][number_of_channels_to_fit_start : number_of_channels_to_fit_end], int(parameters["degree_of_energy_per_channel_polynomial"])))
        ...
        #self.update_dictionary("energy_per_channel_slope_of_elastic_peak", str(exact_energy_per_channel_slope))
        #self.update_dictionary("energy_per_channel_intercept_of_elastic_peak", str(exact_energy_per_channel_intercept))
        ...
        if parameters["is_plot_intensity_normalize_to_value_array"][0]:
            for spectra_index in range(len(array_of_intensity_arrays)):
                maximum_intensity = array_of_intensity_arrays[spectra_index].max()
                value_to_divide_intenisties_with = maximum_intensity/ float(parameters["plot_value_to_normalize_highest_intensity_array"][0])
                array_of_intensity_arrays[spectra_index] = array_of_intensity_arrays[spectra_index]/value_to_divide_intenisties_with
