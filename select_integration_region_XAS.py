...
import get_treated_XAS_data_script
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.setWindowTitle("Simple RIXS select integration region XAS")
        self.show()
        ...
        if self.validate_input(item, key):
            ...
            if int(item_text) != 0:
                for pfy_region in range(int(item_text)):
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*pfy_region+1,self.create_gui_item("pfy_region_name_array_"+ str(pfy_region), "Name of integration region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*pfy_region+2,self.create_gui_item("pfy_incoming_energy_start_array_"+ str(pfy_region), "Lowest excitation energy of integration region "+ str(pfy_region)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*pfy_region+3,self.create_gui_item("pfy_incoming_energy_end_array_"+ str(pfy_region), "Highest excitation energy of integration region "+ str(pfy_region)+ ":                     ", "q_line_edit", [""]))
    ...
    def get_arrays_of_pfy_intensity_and_incoming_energy_arrays(self, parameters):
        ...
        array_of_x_values_arrays, array_of_intenisty_arrays = get_treated_XAS_data_script.get_treated_XAS_data(complete_file_location)
        array_of_integration_values_arrays = []
        for pfy_region in range(int(self.parameters["number_of_pfy_regions"])):
                
            energy_loss_index_start= np.abs(array_of_x_values_arrays[spectra_index] - float(parameters["pfy_incoming_energy_start_array"][pfy_region])).argmin()
            energy_loss_index_end= np.abs(array_of_x_values_arrays[spectra_index] - float(parameters["pfy_incoming_energy_end_array"][pfy_region])).argmin()
            
            for spectra_index in range(len(array_of_intenisty_arrays)):
                #complete_file_location = create_complete_file_location_for_treated_data.create_complete_file_location_for_treated_data(parameters["input_file_project_folder"], parameters["input_complete_file_name_array"][file_name_index])
                #array_of_x_values_arrays, array_of_intenisty_arrays = get_treated_XAS_data_script.get_treated_XAS_data(complete_file_location)
                
                #excitation_energy_index_start= np.abs(arra - float(parameters["pfy_incoming_energy_start_array"][pfy_region])).argmin()
                ...
                #for spectra_index in range(energy_loss_index_start, energy_loss_index_end + 1):
                print("What you see here is not correclty implemented. See the script select_integration_region (not the _XAS one)")
                intensity_array= array_of_intenisty_arrays[spectra_index][energy_loss_index_start:energy_loss_index_end+1]
                total_area= 0
                for datapoint in range(len(intensity_array)):
                    total_area+= intensity_array[datapoint] * array_of_x_values_arrays[spectra_index][datapoint]
                array_of_integration_values.append(total_area)
            array_of_integration_values_arrays.append(array_of_integration_values)
                #array_of_pfy_intensity_arrays.append(total_pfy_region_intensity_array)

        return array_of_integration_values_arrays
    ...
    def print_integration_values(self, parameters):
        ...
        array_of_integration_values_array = self.get_arrays_of_pfy_intensity_and_incoming_energy_arrays(parameters)
        #array_of_integration_values_array = self.rearange_order_of_integration_values(parameters, array_of_integration_values)
        ...
        for integration_region_index in range(int(parameters["number_of_pfy_regions"])):
            ...
            print(array_of_integration_values_array[integration_region_index])
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        array_of_x_value_arrays, array_of_intensity_arrays = get_treated_XAS_data_script.get_treated_XAS_data(complete_file_location)
        ...
        arbitrary_y_axis_to_plot = np.arange(len(array_of_intensity_arrays) + 1)
        arbitrary_y_axis_to_plot = arbitrary_y_axis_to_plot + 0.5
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            im = axs.pcolormesh(array_of_x_value_arrays_to_plot[spectra_index], arbitrary_y_axis_to_plot[spectra_index], array_of_intensity_arrays[spectra_index], cmap=colormap, vmax= max_intensity_value, vmin= min_intensity_value)
        #array_of_energy_loss_arrays= np.zeros(len())
        fig = axs.figure
        cbar = axs.figure.colorbar(im, ax=axs)
        cbar.ax.tick_params(labelsize=14)
        
        axs.set_xlabel("Excitation energy [eV]")
        axs.set_ylabel("Spectra order")
