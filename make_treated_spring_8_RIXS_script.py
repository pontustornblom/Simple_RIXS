#make_treated_spring_8_RIXS_script
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.vbox.addLayout(self.create_gui_item("degree_of_energy_per_channel_polynomial", "What degree polynomial do you want to fit the elastic peaks to? \n(Set to 0 for y= a/(x + b) + c fit, set to 1 for linear) ", "q_line_edit", [""]))        
        ...
        #incoming_energy_array = self.get_incoming_energy_array_spring_8(self.parameters, iteratable_file_number_array)
        ...
        #self.parameters["is_data_x_values_emission_energy"] = True
        ...
        self.setWindowTitle("Simple RIXS Make treated Spring-8 RIXS")
        self.show()
    ...
    def get_incoming_energy_array_spring_8(self, parameters, iteratable_file_number_array):
        #The different options in this function have not been tested thoroughly
        incoming_energy_array = []
    ...
    def get_single_intensity_and_x_axis_spectrum_txt_spring_8(self, parameters, file_name):
        complete_file_location = self.create_complete_file_location_spring_8(parameters, file_name)
        ...
        dataframe = pd.read_csv(complete_file_location, sep=',', skiprows=data_row, usecols=[data_column], header=None, low_memory= False)
        # Access the values of the column
        intensity_array = dataframe.iloc[:-2, 0].values
        
        dataframe = pd.read_csv(complete_file_location, sep=',', skiprows=data_row, usecols=[x_axis_column], header=None, low_memory= False)
        x_axis_array = dataframe.iloc[:-2, 0].values
        return np.array(intensity_array), np.array(x_axis_array)
    
    def get_single_x_values_spectrum_txt_spring_8(self, parameters, iteratable_number):
    ...
    def get_single_i0_spectrum_dat_spring_8(self, parameters, iteratable_number, real_i0_row):
        complete_file_location = self.create_complete_file_location_i0_spring_8(parameters, iteratable_number)
        #omplete_file_location = self.create_complete_file_location_spring_8(parameters, file_name)
  
        data_row = real_i0_row
        data_column= 5
        dataframe = pd.read_csv(complete_file_location, sep='\t', skiprows=data_row, usecols=[data_column], header=None, low_memory= False)
        # Access the values of the column
        i0_array = dataframe.iloc[:, 0].values
        is_multiple_i0_values = False
        if isinstance(i0_array, np.ndarray) or isinstance(i0_array, list):
            if len(i0_array) >1: 
                is_multiple_i0_values = True
                i0_array = i0_array[0]
        
        return np.array(i0_array), is_multiple_i0_values

    def create_complete_file_location_spring_8(self, parameters, file_name):
        if file_name[-4:] != ".txt":
            file_name = file_name + ".txt"
        complete_file_location = os.path.join(parameters["input_file_project_folder"], parameters["input_file_raw_data_folder"], file_name)
    
        return complete_file_location

    def create_complete_file_location_i0_spring_8(self, parameters, iteratable_number):
        i0_file_name = parameters["input_file_text_before_file_number"] + "_" + str(int(iteratable_number)) + "_1.dat"
        complete_file_location = os.path.join(parameters["complete_i0_file_location"], i0_file_name)
    ...
    def get_elastic_peak_channel_center_array_spring_8(self, parameters, approximate_channel_per_energy, array_of_intensity_arrays, incoming_energy_array):
        channels_above_and_below_elastic_to_fit = int(parameters["channels_above_and_below_elastic_to_fit"])
        array_index=0
        elastic_peak_center_array= []
        intensity_weights_array= np.zeros(len(array_of_intensity_arrays))
        intensity_array_index= 0
        for spectra_index in range(len(array_of_intensity_arrays)):
            
            peak_channel_center=0
            sum_of_intensity_weight= 0
            highest_intensity, highest_intensity_channel = find_elastic_peak_maximum_script.find_elastic_peak_maximum(parameters, array_of_intensity_arrays[spectra_index], round(approximate_channel_per_energy*(incoming_energy_array[array_index] - incoming_energy_array[0]) + int(parameters["approximate_channel_of_first_elastic_peak"])))
            if parameters["is_use_converging_weighted_squared_peak_center_finder"]:
                previous_elastic_peak_center_channel = highest_intensity_channel
                condition = True
                while condition:
                ...
                x_values_gaussian= np.linspace(highest_intensity_channel - channels_above_and_below_elastic_to_fit, highest_intensity_channel + channels_above_and_below_elastic_to_fit, 2*channels_above_and_below_elastic_to_fit)
                y_values_gaussian= array_of_intensity_arrays[spectra_index][highest_intensity_channel - channels_above_and_below_elastic_to_fit : highest_intensity_channel + channels_above_and_below_elastic_to_fit]
                mu_guess = highest_intensity_channel
                sigma_guess = (x_values_gaussian[0] - x_values_gaussian[-1]) / 8
                A_guess = highest_intensity
                initial_guesses = [A_guess, mu_guess, sigma_guess]
                ...
                x_values_gaussian= np.linspace(highest_intensity_channel, highest_intensity_channel + channels_above_and_below_elastic_to_fit, channels_above_and_below_elastic_to_fit )
                y_values_gaussian= array_of_intensity_arrays[spectra_index][highest_intensity_channel:highest_intensity_channel + channels_above_and_below_elastic_to_fit]
                #mu_guess = highest_intensity_channel
                #sigma_guess = (x_values_gaussian[0] - x_values_gaussian[-1]) / 4
                #A_guess = highest_intensity / (np.sqrt(2 * np.pi) * sigma_guess)
                #initial_guesses = [A_guess, mu_guess, sigma_guess]
    ...
    def move_elastic_peak_center_to_correct_energy_with_gaussian(self, array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays):
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            try:
                ...
                max_rel_unc_amplitude = 0.5   # σ_A / |A|
                max_rel_unc_center    = 0.5   # σ_μ / |σ_fit|  (center uncertainty vs peak width)
                max_rel_unc_width     = 0.5   # σ_σ / |σ_fit|
    ...
    def move_elastic_peak_center_to_correct_energy_with_gaussian_and_average_over_all_energies(self, array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays):
        ...
        if np.count_nonzero(accepted) == 0:
            ...
            # Birge-ratio sanity check: are individual sigma_mu consistent with the scatter?
    ...
    def move_elastic_peak_center_to_rising_edge_of_elastic_peak(self, array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays):
        #elastic_peak_center_array = np.asarray(elastic_peak_center_array)
        is_original_x_values_energy_loss = self.nested_array_contains_negative_floats(array_of_x_value_arrays)
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            ...
            n_channels_f3 = len(array_of_x_value_arrays[spectra_index])
            half_channel_width_of_elastic_peak = np.abs(array_of_intensity_arrays[spectra_index][previous_elastic_peak_channel_center : ] - rough_half_elastic_peak_intenisty).argmin()
            if half_channel_width_of_elastic_peak <= 1:
                half_channel_width_of_elastic_peak = 2
            channel_of_half_elastic_peak_width_above_elastic = min(previous_elastic_peak_channel_center + half_channel_width_of_elastic_peak, n_channels_f3 - 1)
            half_energy_width_of_elastic_peak = array_of_x_value_arrays[spectra_index][channel_of_half_elastic_peak_width_above_elastic] - array_of_x_value_arrays[spectra_index][previous_elastic_peak_channel_center]
            ...
            condition = True
            iteration_count_f3 = 0
            while condition:
                iteration_count_f3 += 1
                if iteration_count_f3 >= 100:
                    condition = False
                    break
                peak_channel_center = 0
                peak_energy_center = 0
                ...
                channels_above_elastic_to_fit = np.abs(previous_elastic_peak_channel_center - np.abs(array_of_x_value_arrays[spectra_index] - half_energy_width_of_elastic_peak).argmin() )
                channels_below_elastic_to_fit = np.abs(previous_elastic_peak_channel_center - np.abs(array_of_x_value_arrays[spectra_index] + half_energy_width_of_elastic_peak).argmin() )
                for channel in range(max(0, previous_elastic_peak_channel_center - channels_below_elastic_to_fit), min(n_channels_f3, previous_elastic_peak_channel_center + channels_above_elastic_to_fit + 1)):
                    #peak_channel_center+= array_of_intensity_arrays[spectra_index][channel] * channel
                    #sum_of_intensity_weight+= array_of_intensity_arrays[spectra_index][channel]
                    #peak_channel_center+= ((array_of_intensity_arrays[spectra_index][channel]) ** 2 ) * channel
                    peak_energy_center+= ((array_of_intensity_arrays[spectra_index][channel]) ** 2 ) * array_of_x_value_arrays[spectra_index][channel]
        ...
        #simply align them at the rising edge of the elastig, which should give all spectra the same systematic error but facilitate comparisons:
        for spectra_index in range(len(array_of_intensity_arrays)):
            try:
                previous_elastic_peak_channel_center = np.abs(array_of_x_value_arrays[spectra_index] - 0).argmin()
                #channels_above_and_below_elastic_to_fit = np.abs(previous_elastic_peak_channel_center - np.abs(array_of_x_value_arrays[spectra_index] - energy_above_and_below_elastic_peak_to_fit_elastic_peak).argmin() )
                ...
                half_energy_width_of_elastic_peak = array_of_x_value_arrays[spectra_index][channel_of_half_elastic_peak_width_above_elastic] - array_of_x_value_arrays[spectra_index][previous_elastic_peak_channel_center]
                half_energy_width_of_elastic_peak = min(half_energy_width_of_elastic_peak,
                                            10*energy_above_and_below_elastic_peak_to_fit_elastic_peak)
                
                array_of_x_value_arrays[spectra_index] = array_of_x_value_arrays[spectra_index] - half_energy_width_of_elastic_peak
                
            except RuntimeError:
                print("Shift to rising edge of elastic peak could not be made for spectra: ", spectra_index)
    ...
    def get_array_of_intensity_arrays_spring_8(self, parameters, iteratable_file_number_array, iteratable_file_number_array_i0,  list_of_filenames, list_of_excitation_energies):
        array_of_intensity_arrays= []
        array_of_x_value_arrays = []

        #array_of_x_axis_arrays = []
        real_i0_iteratable_number = iteratable_file_number_array_i0[0]
        real_i0_index = 0
        original_i0_data_row = 5
        real_i0_data_row = original_i0_data_row
        for spectra_index in range(len(iteratable_file_number_array)):
            intensity_array, x_values_array= self.get_single_intensity_and_x_axis_spectrum_txt_spring_8(parameters, list_of_filenames[spectra_index])
            if x_values_array[0] > x_values_array[-1]:
                #if parameters["is_input_energy_per_channel_and_intercept_manually"] == True or parameters["is_x_values_available_in_file"] == True:
                x_values_array = np.flip(x_values_array)
                intensity_array = np.flip(intensity_array)
            #if parameters["is_input_energy_per_channel_and_intercept_manually"]:
            #    x_values_array = [x * exact_energy_per_channel_slope + exact_energy_per_channel_intercept for x in range(len(intensity_array))]
            ...
            if parameters["is_normalization_to_i0"]:
                i0_values, is_multiple_i0_values = self.get_single_i0_spectrum_dat_spring_8(parameters, real_i0_iteratable_number, real_i0_data_row)
                i0_sum= np.sum(i0_values)
                
                if i0_sum != 0:
                    intensity_array= intensity_array/i0_sum
                #is_multiple_i0_values = True
                if is_multiple_i0_values == False:
                    if spectra_index != len(iteratable_file_number_array) -1:
                        real_i0_index += 1
                        real_i0_iteratable_number = iteratable_file_number_array_i0[real_i0_index]
                        #real_i0_iteratable_number == str(int(real_i0_iteratable_number) + 1)
                        real_i0_data_row = original_i0_data_row
                else:
                    real_i0_data_row += 1
        ...
        #The for loop below is to combine spectra with the same excitation energy. This is because we did several measurements with the same excitation energy to minimize beam damage
        #unique_excitation_energies_list = list(set(list_of_excitation_energies))
        used_excitation_energies = set()

        #temporary two lines below, good for XES
        #list_of_excitation_energies_2 = list_of_excitation_energies.copy()
        #list_of_excitation_energies = [389.95 for x in list_of_excitation_energies_2]

        unique_excitation_energies_list = [x for x in list_of_excitation_energies if x not in used_excitation_energies and (used_excitation_energies.add(x) or True)]
        
        
        array_of_combined_intensity_arrays= []
        spectra_index_original_list = 0
        list_of_excitation_energies = list(list_of_excitation_energies)
        for excitation_energy in unique_excitation_energies_list:
            indices_of_same_energy = [i for i, x in enumerate(list_of_excitation_energies) if x == excitation_energy]
            combined_intensity_array = np.zeros(len(x_values_array), dtype=object)
            remove_extra_x_values_array_counter = 0
            for spectra_index in indices_of_same_energy:
                combined_intensity_array += array_of_intensity_arrays[spectra_index_original_list]
                if remove_extra_x_values_array_counter >0:
                    del array_of_x_value_arrays[indices_of_same_energy[0] + 1]
                    del list_of_excitation_energies[indices_of_same_energy[0] + 1]
                remove_extra_x_values_array_counter += 1
                spectra_index_original_list += 1
            array_of_combined_intensity_arrays.append(combined_intensity_array)
        
        array_of_intensity_arrays = array_of_combined_intensity_arrays

        
        if parameters["is_input_energy_per_channel_and_intercept_manually"] == False and parameters["is_x_values_available_in_file"] == False:
            #script that finds elastic peak based on these intensity arrays and gives out an array_of_x_value_arrays
            print("I have now made it so the x and intensity arrays are flipped if the last value is larger, so these approximate channels will be incorrect unless changed in previous script!")
            ...
            if unique_excitation_energies_list[0] > unique_excitation_energies_list[-1]:
                unique_excitation_energies_list = np.flip(unique_excitation_energies_list)
                array_of_intensity_arrays = np.flip(array_of_intensity_arrays, axis=0)
                array_of_x_value_arrays = np.flip(array_of_x_value_arrays, axis=0)

            approximate_channel_per_energy= (first_spectra_approximate_channel - last_spectra_approximate_channel)/(unique_excitation_energies_list[0] - unique_excitation_energies_list[-1])
            
            elastic_peak_center_array, array_of_intensity_arrays, intensity_weights_array= self.get_elastic_peak_channel_center_array_spring_8(parameters, approximate_channel_per_energy, array_of_intensity_arrays, unique_excitation_energies_list)

            if parameters["is_intensity_weight_for_linear_fit"]:
                polynomial_coefficients = self.weighted_polynomial_fit(elastic_peak_center_array, unique_excitation_energies_list, intensity_weights_array)
            else:
                polynomial_coefficients = self.polynomial_fit(elastic_peak_center_array, unique_excitation_energies_list)



            if self.parameters["degree_of_energy_per_channel_polynomial"] == "0":
                elastic_peak_center_array_channels = np.arange(1, len(elastic_peak_center_array) + 1)
                initial_guess_a = max(elastic_peak_center_array) - min(elastic_peak_center_array)  # Estimate for parameter 'a' based on the range of y values
                initial_guess_b = elastic_peak_center_array_channels[np.argmin(elastic_peak_center_array)]  # Estimate for parameter 'b' based on the x value corresponding to the minimum y value
                initial_guess_c = min(elastic_peak_center_array)
                initial_guess = [initial_guess_a, initial_guess_b, initial_guess_c]  # Initial guess for the parameters a, b, and c
                optimized_params, _ = curve_fit(self.one_over_x_function, np.arange(1, len(elastic_peak_center_array) + 1), elastic_peak_center_array, p0=initial_guess)
                ...
                ax1.plot(elastic_peak_center_array_channels, self.one_over_x_function(elastic_peak_center_array_channels, optimized_a, optimized_b, optimized_c), label=f'Fitted Curve: y = {optimized_a}/(x + {optimized_b}) + {optimized_c}', color='red')
                ax1.plot(np.arange(len(elastic_peak_center_array)), elastic_peak_center_array, color= 'blue', label= "Actual energy per channel of x axis")
                ax1.title('Energy per channel of x axis for the first spectra and its y= a/(x+b) + c fit')
                ax1.xlabel('Channel')
                ax1.ylabel('Energy [eV]')
                ax1.show()
            else:
                #polynomial_coefficients = self.weighted_polynomial_fit_for_x_axis(np.arange(len(elastic_peak_center_array)), elastic_peak_center_array)
                
                y1 = np.polyval(polynomial_coefficients, elastic_peak_center_array)
                y2 = unique_excitation_energies_list
                y_2_1_difference = np.subtract(y2, y1)
                figure, (ax1, ax2) = plt.subplots(2, 1)
                ax1.plot(elastic_peak_center_array, y1, color = 'red', label= "Fitted polynomial")
                ax1.plot(elastic_peak_center_array, y2, color= 'blue', label= "Actual energy per channel of x axis")
                ax1.set_title('Energy per channel of the elastic peak positions and its polynomial fit')
                ax1.set_xlabel('Channel')
                ax1.set_ylabel('Energy [eV]')
                ax1.legend()
                #ax2.twinax(ax1)
                ax2.plot(unique_excitation_energies_list, y_2_1_difference)
                ax2.set_title('Difference between energy per channel of x axis\nand its polynomial fit')
        ...
        return np.array(array_of_intensity_arrays), np.array(array_of_x_value_arrays), np.array(unique_excitation_energies_list)
    ...
    def get_x_values_array_and_energy_per_channel(self, array_of_x_values_array, array_of_intensity_arrays, iteratable_file_number_array, incoming_energy_array):
        ...
        for spectrum in range(len(array_of_x_values_array)):
        ...
        if self.parameters["is_input_energy_per_channel_and_intercept_manually"]:
            ...
            exact_energy_per_channel_intercept = float(self.parameters["energy_per_channel_intercept_of_elastic_peak"])
        else:
            x_array_index_start= np.abs(array_of_x_values_array[0] - incoming_energy_array[0]).argmin()
            x_array_index_end= np.abs(array_of_x_values_array[0] - incoming_energy_array[-1]).argmin()
            exact_energy_per_channel_slope= (array_of_x_values_array[0][x_array_index_end]- array_of_x_values_array[0][x_array_index_start])/len(array_of_x_values_array[0][x_array_index_end : x_array_index_start])
            exact_energy_per_channel_intercept = incoming_energy_array[0] - exact_energy_per_channel_slope * x_array_index_start
            #exact_energy_per_channel_slope= sum_of_average_energy_per_pixel/len(array_of_x_values_array)
        ...
        array_of_x_values_array = array_of_x_values_array.tolist()
        for spectrum in range(len(array_of_x_values_array)):
            condition= True
            counter = 0
            while condition:
                counter += 1
                if total_highest_x_value <= array_of_x_values_array[spectrum][0] - half_of_exact_energy_per_channel_slope:
                    #np.insert(array_of_x_values_array[spectrum], 0, (array_of_x_values_array[spectrum][0] - exact_energy_per_channel_slope))
                    #array_of_x_values_array[spectrum] = np.concatenate([array_of_x_values_array[spectrum][0] - exact_energy_per_channel_slope], array_of_x_values_array[spectrum])
                    #array_of_x_values_array[spectrum] = np.concatenate([[array_of_x_values_array[spectrum][0] - exact_energy_per_channel_slope], array_of_x_values_array[spectrum]])
            ...
            condition= True
            while condition:
                if total_lowest_x_value>= array_of_x_values_array[spectrum][-1] + half_of_exact_energy_per_channel_slope:
        ...
        for map_index in range(len(array_of_x_values_array)):
            sum_of_lowest_x_values+= array_of_x_values_array[map_index][0]
            sum_of_highest_x_values+= array_of_x_values_array[map_index][-1]
        total_lowest_x_value= sum_of_lowest_x_values/len(array_of_x_values_array)
        total_highest_x_value= sum_of_highest_x_values/len(array_of_x_values_array)
        
        print("THIS SHOULD NOT DO A LINEAR APPROX OF THE X_AXIS IF THE DATA IS ALREADY Emission energy")
        #final_x_values_array= np.linspace(total_lowest_x_value, total_highest_x_value, len(array_of_intensity_arrays[0]))
        
        return array_of_x_values_array[0], exact_energy_per_channel_slope, exact_energy_per_channel_intercept, array_of_intensity_arrays
        ...
        for spectra_index in range(len(array_of_intensity_arrays)):
            ...
            while condition:
                ...
                for channel in range(previous_elastic_peak_channel_center - channels_below_elastic_to_fit, previous_elastic_peak_channel_center + channels_above_elastic_to_fit + 1):
                    ...
                    peak_energy_center+= ((array_of_intensity_arrays[spectra_index][channel]) ** 2 ) * array_of_x_value_arrays[spectra_index][channel]
                    sum_of_intensity_weight+= (array_of_intensity_arrays[spectra_index][channel]) ** 2
                    #peak_energy_center+= ((array_of_intensity_arrays[spectra_index][channel])) * array_of_x_value_arrays[spectra_index][channel]
                    #sum_of_intensity_weight+= (array_of_intensity_arrays[spectra_index][channel])
    ...
    def weighted_linear_fit(self, x, y, weights):
        popt, pcov = curve_fit(self.linear_function, x, y, sigma=(max(weights)/weights))
        return popt
    ...
    def get_file_name_and_excitation_energy_lists_spring_8(self, parameters, iteratable_file_number_array):
        file_folder_location = os.path.join(parameters["input_file_project_folder"], parameters["input_file_raw_data_folder"])
        list_of_filenames = []
        list_of_excitation_energies= []
        iterateable_file_number_index = 0
        #While loop because the files may not be in the same order as in the iteratable file number array
        condition = True
        while condition:
            for file in os.listdir(file_folder_location):
                if os.path.isfile(os.path.join(file_folder_location, file)) and parameters["input_file_text_before_file_number"] in file:
                    if file[-11:] == ".dat_cc.txt":
                        split_file_list = file.split('_')
                        iteration_number_split = split_file_list[-2]
                        excitation_energy_split = split_file_list[-4]
                        #iteration_number_split = '_'.join(split_file_list[:-1])
                        file_name_split = '_'.join(split_file_list[1:-4])
                        if parameters["input_file_text_before_file_number"] != file_name_split:
                            continue
                        
                        pattern = r'\d{1,20}'  # Match sequences of 1 to 20 digits
                        match = re.search(pattern, iteration_number_split)
                        if match:
                            iteration_number= match.group()

                        if int(iteration_number) != int(iteratable_file_number_array[iterateable_file_number_index]):
                            continue
                            #parameters["input_file_number_of_files_to_ignore"] > 0:
                        
                        pattern = r'\d+(\.\d+)?'
                        match = re.search(pattern, excitation_energy_split)     
                        if match:
                            excitation_energy= float(match.group())
                        else:
                            pattern = r'\d{1,20}'  # Match sequences of 1 to 20 digits
                            match = re.search(pattern, excitation_energy_split)
                            if match:
                        ...
                        list_of_excitation_energies.append(excitation_energy)

                        iterateable_file_number_index += 1

                        if len(list_of_filenames) == len(iteratable_file_number_array):
                            condition = False
                            break
        

        return list_of_filenames, list_of_excitation_energies
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        if parameters["is_input_file_names_manually"] == False:
            ...
            if parameters["is_normalization_to_i0"]:
                iteratable_file_number_array_i0= self.get_iteratable_file_number_array_2(parameters) #[x for x in range(len(iteratable_file_number_array))]    #self.get_iteratable_file_number_array_2(parameters)
            else:
                iteratable_file_number_array_i0 =[0]
            list_of_filenames, list_of_excitation_energies = self.get_file_name_and_excitation_energy_lists_spring_8(parameters, iteratable_file_number_array)
            #incoming_energy_array = self.get_incoming_energy_array_spring_8(parameters, iteratable_file_number_array)
            list_of_excitation_energies = np.asarray(list_of_excitation_energies)
            list_of_filenames = np.asarray(list_of_filenames)
            iteratable_file_number_array = np.asarray(iteratable_file_number_array)
            iteratable_file_number_array_i0 = np.asarray(iteratable_file_number_array_i0)
            
            sorted_indices = np.argsort(list_of_excitation_energies)
            list_of_excitation_energies = list_of_excitation_energies[sorted_indices]
            list_of_filenames = list_of_filenames[sorted_indices]
            iteratable_file_number_array = iteratable_file_number_array[sorted_indices]
            if parameters["is_normalization_to_i0"]:
                iteratable_file_number_array_i0 = iteratable_file_number_array_i0[sorted_indices]
            else:    
                iteratable_file_number_array_i0 =[0]

            if False:
                if list_of_excitation_energies[0] > list_of_excitation_energies[-1]:
                    list_of_excitation_energies = np.flip(list_of_excitation_energies)
                    list_of_filenames = np.flip(list_of_filenames)
                    iteratable_file_number_array = np.flip(iteratable_file_number_array)
                    iteratable_file_number_array_i0 = np.flip(iteratable_file_number_array_i0)
            ...
            array_of_intensity_arrays, array_of_x_value_arrays, incoming_energy_array = self.get_array_of_intensity_arrays_spring_8(parameters, iteratable_file_number_array, iteratable_file_number_array_i0, list_of_filenames, list_of_excitation_energies)
            ...
            if parameters["is_normalization_to_i0"]:
                ...
                iteratable_file_number_array_i0 =[0]
            #list_of_filenames, list_of_excitation_energies = self.get_file_name_and_excitation_energy_lists_spring_8(parameters, iteratable_file_number_array)
            list_of_filenames = parameters["input_complete_file_name_array"]
            list_of_excitation_energies = self.get_incoming_energy_array(parameters, iteratable_file_number_array)
        ...
        #Rearange the order of the arrays so they are in correct order of the incoming energy 
        #sorted_indices = np.argsort(incoming_energy_array)
        #incoming_energy_array = incoming_energy_array[sorted_indices]
        #array_of_intensity_arrays = array_of_intensity_arrays[sorted_indices]
        #array_of_x_value_arrays = array_of_x_value_arrays[sorted_indices]

        #if incoming_energy_array[0] > incoming_energy_array[-1]:
        #    incoming_energy_array = np.flip(incoming_energy_array)
        #    array_of_intensity_arrays = np.flip(array_of_intensity_arrays, axis=0)
        #    array_of_x_value_arrays = np.flip(array_of_x_value_arrays, axis=0)
        #x_values_array, exact_energy_per_channel_slope, exact_energy_per_channel_intercept, array_of_intensity_arrays = self.get_x_values_array_and_energy_per_channel(array_of_x_value_arrays, array_of_intensity_arrays, iteratable_file_number_array, incoming_energy_array)
        ...
        #exact_energy_per_channel_slope, exact_energy_per_channel_intercept, exact_energy_per_channel_r_value, exact_energy_per_channel_p_value, exact_energy_per_channel_std_err = stats.linregress(energy_calibration_channel_position_array, energy_calibration_energy_position_array)
        ...
        if parameters["is_automatically_adjust_peak_to_correct_energy"]:
            ...
            #test below
            print("OBS: Currently using a different way of aligning the elastic peaks. We are shifting all elastic peaks based on weighted average of all spectra.")
            ...
            #This one below was a test, dont use it
            #array_of_x_value_arrays = self.move_elastic_peak_center_to_rising_edge_of_elastic_peak(array_of_intensity_arrays, incoming_energy_array, array_of_x_value_arrays)
        ...
        if parameters["plot_invert_y_axis"]:
            if parameters["plot_waterfall_instead_of_heat_map"]== True:
                ...
                reversed_array_of_x_value_arrays = np.flip(array_of_x_value_arrays, axis=0)
            else:
                ax.invert_yaxis()
        ...
        if parameters["plot_waterfall_instead_of_heat_map"]== True:
            if parameters["plot_invert_y_axis"]== False:
                ...
                for spectra in range(len(incoming_energy_array)):
                    ax.plot(reversed_array_of_x_value_arrays[spectra], reversed_array_of_intensity_arrays[spectra] + float(parameters["plot_y_distance_between_plots"]) * spectra)
                    if parameters["plot_display_incoming_energy_by_lines"]:
            ...
            # Decide on the intensity limits before plotting
            ...
            # Build a single shared norm so the colorbar controls all pcolormesh patches
            if parameters["is_log_scale_color_bar"]:
                if vmin_val <= 0:
                    vmin_val = vmax_val * 0.01 if vmax_val < 100 else 1
