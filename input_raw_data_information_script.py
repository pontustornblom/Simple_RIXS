#input_raw_data_information_script
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        if self.parameters["plot_type"]== "Make treated RIXS data" or self.parameters["plot_type"]== "Make treated RIXS data from Soleil Galaxies":
            ...
            self.vbox.addLayout(self.create_gui_item("input_file_text_before_file_number", "Text before iteratable file number: ", "q_line_edit", [""]))
            ...
            self.vbox.addLayout(self.create_gui_item("is_multiple_detectors", "Are there multiple detectors? (This is not implemented yet) ", "q_check_box", [""]))
            if self.parameters["is_multiple_detectors"]:
                self.vbox.addLayout(self.create_gui_item("input_file_text_before_detector", "Text between iteratable file number and detector number: ", "q_line_edit", [""]))
                self.vbox.addLayout(self.create_gui_item("input_file_number_of_detectors", "Number of detectors: ", "q_line_edit", [""]))
                self.is_multitiple_detectors_input_displayed= True 
            #self.vbox.addLayout(self.create_gui_item("input_file_text_before_detector", "Text between iteratable file number and detector number: ", "q_line_edit", [""]))
            #self.vbox.addLayout(self.create_gui_item("input_file_detector_numbers", "Input file text before iteratable file number: ", "q_line_edit", [""]))
            self.vbox.addLayout(self.create_gui_item("input_file_text_end_of_name", "Text at the end of the file: ", "q_line_edit", [""]))
        ...
        if self.parameters["is_i0_avialable_in_seperate_file"] and self.parameters["is_view_roots_or_input_txt"] == False:
            self.vbox.addLayout(self.create_gui_item("", "You have previously inputted that the i0 (i-zero) is in another file. Please give the file location and name of that file below.", "q_text_label", [""]))
        ...
        self.setWindowTitle("Simple RIXS Input raw data information")
        self.show()
