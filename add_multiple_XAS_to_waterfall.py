...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.setWindowTitle("Simple RIXS Add multiple XAS to waterfall")
        self.show()
    ...
    def create_multiple_gui_items_from_checkbox_arrays(self, key, array_index, item, hbox):
        ...
        if key == "is_approximate_energy_for_normalization_to_zero_array":
            if self.parameters["is_approximate_energy_for_normalization_to_zero_array"][array_index]:
                ...
                self.remove_item(self.vbox.indexOf(hbox)+1)
                self.remove_item(self.vbox.indexOf(hbox)+2)
        #This background subtraction below has been replaced
    ...
    def treat_intensity_data(self, parameters, array_of_pfy_intensity_arrays, array_of_pfy_incoming_energy_arrays):
        ...
        for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
            ...
            if False and parameters["is_invert_the_plot_array"][array_index]== True:
                ...
                if False and parameters["is_normalize_to_zero_and_one_array"][array_index]:
                    if parameters["is_approximate_energy_for_normalization_to_zero_array"][array_index]:
                        index_start = np.abs(incoming_energy_array - (float(parameters["approximate_energy_for_normalization_to_zero_array"][array_index]) - float(parameters["energy_above_and_below_finding_min_intensity_array"][array_index]))).argmin()
                        index_end = np.abs(incoming_energy_array - (float(parameters["approximate_energy_for_normalization_to_zero_array"][array_index]) + float(parameters["energy_above_and_below_finding_min_intensity_array"][array_index]))).argmin()
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        plt.close()
        self.array_of_x_values_arrays = []
        self.array_of_intensity_arrays = []
        input_file_project_folder = parameters["input_file_project_folder"]
        for spectra_index in range(int(parameters["input_number_of_complete_file_names"])):
            complete_file_name = parameters["input_complete_file_name_array"][spectra_index]
            complete_file_location = create_complete_file_location_for_treated_data.create_complete_file_location_for_treated_data(input_file_project_folder, complete_file_name)
            incoming_energy_array, intensity_array = get_treated_XAS_data_script.get_treated_XAS_data(complete_file_location)
            self.array_of_x_values_arrays.append(incoming_energy_array[0])
            self.array_of_intensity_arrays.append(intensity_array[0])
        ...
        self.array_of_intensity_arrays = self.treat_intensity_data(parameters, self.array_of_intensity_arrays, self.array_of_x_values_arrays)
        ...
        if parameters["plot_spectra_grouping_type"]== "No grouping":
            ...
            for file_name_index in range(int(self.parameters["input_number_of_complete_file_names"])):
                ...
                if parameters["is_hide_certain_spectra_array"][array_index] == False:
                    ...
                    ax.plot(array_of_x_values_arrays_to_plot[array_index], array_of_intensity_arrays_to_plot[array_index] + float(parameters["plot_y_distance_between_plots"]) * file_name_index, label = parameters["plot_legend_names_array"][file_name_index], color=plot_color_array[array_index], linestyle= linestyles_array[array_index])
                    #else:
                    #    ax.plot(array_of_x_values_arrays_to_plot[array_index], array_of_intensity_arrays_to_plot[array_index] + float(parameters["plot_y_distance_between_plots"]) * file_name_index, color=plot_color_array[array_index], linestyle= linestyles_array[array_index])
