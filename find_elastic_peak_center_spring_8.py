#find_elastic_peak_center_spring_8
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.vbox.addLayout(self.create_gui_item("is_input_energy_per_channel_and_intercept_manually", "Would you like to manually input the energy per channel and intercept of the line that goes through the elastic peaks? \n(You will input this in a later window) ", "q_check_box", [""]))
        ...
        self.vbox.addLayout(self.create_gui_item("is_intensity_weight_for_linear_fit", "Would you like the linear fit to the elastic peak centers to be weighted by the intensity of the elastic peak?\n(This might not work with too few spectral lines) ", "q_check_box", [""]))
        ...
        self.setWindowTitle("Simple RIXS Find Elastic Peak SPring-8")
        self.show()
    ...
    def save_and_continue(self):
        ...
        plt.close()
        #plt.gcf() Har inte testat detta fullt ut men enbart detta fungerade inte
        self.finished.emit()
        self.close()
    ...
    def get_approximate_channel_per_energy_and_incoming_energy_array_spring_8(self, parameters, incoming_energy_array):
        ...
        return approximate_channel_per_energy
    ...
    def get_elastic_peak_channel_center_array(self, parameters, approximate_channel_per_energy, list_of_filenames, incoming_energy_array, array_of_intensity_arrays):
        channels_above_and_below_elastic_to_fit = int(parameters["channels_above_and_below_elastic_to_fit"])

        array_index=0
        elastic_peak_center_array= []
        #array_of_intensity_arrays= np.zeros(len(list_of_filenames), dtype=object)
        intensity_weights_array= np.zeros(len(list_of_filenames))
        intensity_array_index= 0
        array_of_intensity_arrays = list(array_of_intensity_arrays)
        for intensity_array in array_of_intensity_arrays:
            #iteratable_int= iteratable_number_to_int_script.iteratable_number_to_int(iteratable_number)
            #y_values = self.get_single_intensity_and_x_axis_spectrum_txt_spring_8(parameters, file_name)
            y_values = intensity_array
            if False:
    ...
    def get_array_of_intensity_arrays_spring_8(self, parameters, iteratable_file_number_array, iteratable_file_number_array_i0,  list_of_filenames, list_of_excitation_energies):
        ...
        #The for loop below is to combine spectra with the same excitation energy.
        ...
        if False:
            original_length = len(array_of_combined_intensity_arrays[0])
            desired_length = original_length * 10 - 9
            for spectra_index in range(len(array_of_combined_intensity_arrays)):
                x_values = np.linspace(0, original_length - 1, desired_length)
                old_y_values = np.array(array_of_combined_intensity_arrays[spectra_index], dtype=float)
                y_values = np.interp(x_values, np.arange(original_length), old_y_values)
                x_values = np.arange(0, len(y_values))
                array_of_combined_intensity_arrays[spectra_index] = y_values
                array_of_x_value_arrays[spectra_index] = x_values

        return np.array(array_of_combined_intensity_arrays), np.array(array_of_x_value_arrays), np.array(unique_excitation_energies_list)
    ...
    def get_single_intensity_and_x_axis_spectrum_txt_spring_8(self, parameters, file_name):
        ...
        dataframe = pd.read_csv(complete_file_location, sep=',', skiprows=data_row, usecols=[x_axis_column], header=None)
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        #iteratable_file_number_array_i0= [x for x in range(len(iteratable_file_number_array))]    #self.get_iteratable_file_number_array_2(parameters)
        ...
        x_values = [x for x in range(len(array_of_intensity_arrays[0]))]
        array_of_intensity_arrays = np.vstack(array_of_intensity_arrays).astype(float)

        plots= []
        plots.append([x_values, array_of_intensity_arrays[0], "Channel", "Counts" ])
        plots.append([x_values, array_of_intensity_arrays[-1], "Channel", "Counts" ])
        ...
        #iteratable_file_number_array= self.get_iteratable_file_number_array(parameters)
        #iteratable_file_number_array_i0= [x for x in range(len(iteratable_file_number_array))] #added this line to adapt to Spring-8 data

        approximate_channel_per_energy = self.get_approximate_channel_per_energy_and_incoming_energy_array_spring_8(parameters, incoming_energy_array)
        ...
        elastic_peak_center_array, x_values, array_of_intensity_arrays, intensity_weights_array= self.get_elastic_peak_channel_center_array(parameters, approximate_channel_per_energy, list_of_filenames, incoming_energy_array, array_of_intensity_arrays)
