#select_detector_range_galaxies
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.vbox.addLayout(self.create_gui_item("detector_range_start", "Input the pixel number where the integration should start: ", "q_line_edit", [""]))
        self.vbox.addLayout(self.create_gui_item("detector_range_end", "Input the pixel number where the integration should end: ", "q_line_edit", [""]))
        ...
        self.setWindowTitle("Simple RIXS select detector range Galaxies")
        self.show()
    ...
    def get_single_intensity_spectrum_h5_galaxies(self, parameters, complete_file_location):
        ...
        with h5py.File(complete_file_location, 'r') as file:
            try:
                ...
                raw_data_list = raw_data_list[0][0][:][:]
    ...
    def get_array_of_intensity_arrays_galaxies(self, parameters, iteratable_file_number_array, iteratable_file_number_array_i0):
        array_index=0
        array_of_intensity_arrays= np.zeros(len(iteratable_file_number_array), dtype=object)
        intensity_array_index= 0
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        parameters, complete_file_location = create_complete_file_location_view_roots_or_txt_script.create_complete_file_location_view_roots_or_txt(self.parameters, True, iteratable_file_number_array[0])

        array_of_intensity_arrays = self.get_single_intensity_spectrum_h5_galaxies(parameters, complete_file_location)
        ...
        incoming_energy_array = np.arange(len(array_of_intensity_arrays))
        for iteratable_number in range(len(array_of_intensity_arrays)):
        ...
        if parameters["is_plot_intensity_limits_used_array"][array_index] and parameters["plot_waterfall_instead_of_heat_map"]== False:
            ...
            for spectra_index in range(len(array_of_intensity_arrays)):
                #incoming_energy = incoming_energy_array_to_plot[spectra_index : spectra_index + 2]
                print(array_of_x_value_arrays_to_plot[spectra_index].shape)
                print(incoming_energy_array_to_plot[spectra_index : spectra_index + 2].shape)
                print(array_of_intensity_arrays[spectra_index].shape)
        ...
        try:
            energy_loss_start = float(parameters["detector_range_start"])
            incoming_energy_start = float(incoming_energy_array[0] - 0.5 * (incoming_energy_array[1] - incoming_energy_array[0]))
            energy_loss_end = float(parameters["detector_range_end"])
            incoming_energy_end = float(incoming_energy_array[-1] + 0.5 * (incoming_energy_array[-1] - incoming_energy_array[-2]))
            rect = Rectangle((energy_loss_start, incoming_energy_start), energy_loss_end - energy_loss_start, incoming_energy_end - incoming_energy_start, facecolor='none', edgecolor=rectangle_colors_array[0], linewidth= 3)
