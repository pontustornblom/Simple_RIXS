...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.vbox.addLayout(self.create_gui_item("", " You previously prompted that the incoming energy is not available in the datafile.\n You therefore have to input information about the incoming energy below. \n Please check one of the boxes below and input the appropriate values. ", "q_text_label", [""]))
        
        self.vbox.addLayout(self.create_gui_item("is_equal_incoming_energy_difference", "Are the spectra taken with equal energy difference in the incoming energy? ", "q_check_box", [""]))
        self.create_dynamic_gui_item_from_checkboxes(self.item_is_equal, self.item_is_equal.text(), "is_equal_incoming_energy_difference", self.hbox_is_equal, "input_first_and_last_incoming_energy", "Input the incoming energy of the first and last spectra: ")
        
        self.vbox.addLayout(self.create_gui_item("is_segments_of_equal_incoming_energy_difference", "Are the spectra taken with several segments of equal energy difference in the incoming energy? ", "q_check_box", [""]))
        self.create_dynamic_gui_item_from_checkboxes(self.item_is_segments, self.item_is_segments.text(), "is_segments_of_equal_incoming_energy_difference", self.hbox_is_segments, "input_number_of_incoming_energy_segments", "How many segments of equal incoming energy difference are there? \n (15 max) ")
        if self.parameters["is_segments_of_equal_incoming_energy_difference"]:
            self.create_dynamic_gui_item_for_segments(self.item_number_of_segments, self.item_number_of_segments.text(), "input_number_of_incoming_energy_segments", self.hbox_number_of_segments, "input_number_of_incoming_energy_segments", "How many segments of equal incoming energy difference are there? \n(Maximum 10 segments)")       
        
        self.vbox.addLayout(self.create_gui_item("is_segments_of_equal_incoming_energy_difference_with_gap", "Are the spectra taken with several segments of equal energy difference in the incoming energy\nand with some gap in the incoming energies? ", "q_check_box", [""]))
        self.create_dynamic_gui_item_from_checkboxes(self.item_is_segments_with_gap, self.item_is_segments_with_gap.text(), "is_segments_of_equal_incoming_energy_difference_with_gap", self.hbox_is_segments_with_gap, "input_number_of_incoming_energy_segments_with_gap", "How many segments of equal incoming energy difference are there? \n (15 max) ")
        if self.parameters["is_segments_of_equal_incoming_energy_difference_with_gap"]:
            self.create_dynamic_gui_item_for_segments_with_gap(self.item_number_of_segments_with_gap, self.item_number_of_segments_with_gap.text(), "input_number_of_incoming_energy_segments_with_gap", self.hbox_number_of_segments_with_gap, "input_number_of_incoming_energy_segments_with_gap", "How many segments of equal incoming energy difference are there? \n(Maximum 10 segments)")       

        self.vbox.addLayout(self.create_gui_item("is_input_every_incoming_energy", "Would you like to input the incoming energy for each spectra? ", "q_check_box", [""]))
        self.create_dynamic_gui_item_from_checkboxes(self.item_input_every_energy, self.item_input_every_energy.text(), "is_input_every_incoming_energy", self.hbox_input_every_energy, "input_number_of_incoming_energies", "How many spectra would you like to give the incoming energy for? \n (Maximum 50 energies) ")
        if self.parameters["is_input_every_incoming_energy"]:
            self.create_dynamic_gui_item_for_incoming_energies(self.item_number_of_incoming_energies, self.item_number_of_incoming_energies.text(), "input_number_of_incoming_energies", self.hbox_number_of_incoming_energies, "input_number_of_incoming_energies", "How many spectra would you like to give the incoming energy for? \n (Maximum 30 energies)")
        ...
        self.setWindowTitle("Simple RIXS Input incoming Energy")
        self.show()
        ...
        if self.validate_input(item, key):
            ...
            if int(item_text) != 0:
                for segment in range(int(item_text)):
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+2*segment+1,self.create_gui_item("first_incoming_energy_of_segment_array_"+ str(segment), "First incoming energy of segment "+ str(segment)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+2*segment+2,self.create_gui_item("incoming_energy_difference_in_segment_array_"+ str(segment), "Incoming energy difference of spectra in segment "+ str(segment)+ ":", "q_line_edit", [""]))
                self.vbox.insertLayout(self.vbox.indexOf(hbox)+2*int(item_text)+1,self.create_gui_item("last_incoming_energy_of_segment_array_0", "Incoming energy of last spectra: ", "q_line_edit", [""]))
    ...
    def create_dynamic_gui_item_for_segments_with_gap(self, item, item_text, key, hbox, new_key, new_label_text):
        if self.validate_input(item, key):
            ...
            if int(item_text) != 0:
                for segment in range(int(item_text)):
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*segment+1,self.create_gui_item("first_incoming_energy_of_segment_array_"+ str(segment), "First incoming energy of segment "+ str(segment)+":                     ", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*segment+2,self.create_gui_item("incoming_energy_difference_in_segment_array_"+ str(segment), "Incoming energy difference of spectra in segment "+ str(segment)+ ":", "q_line_edit", [""]))
                    self.vbox.insertLayout(self.vbox.indexOf(hbox)+3*segment+3,self.create_gui_item("last_incoming_energy_of_segment_array_"+ str(segment), "Last incoming energy of segment "+ str(segment)+":                     ", "q_line_edit", [""]))
