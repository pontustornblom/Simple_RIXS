#find_elastic_peak_center_galaxies
import sys
import os
import platform
import subprocess
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("Simple RIXS Find Elastic Peak Galaxies")
        self.show()
    ...
    def get_elastic_peak_channel_center_array(self, parameters, approximate_channel_per_energy, iteratable_file_number_array, incoming_energy_array ):
        ...
        for iteratable_number in iteratable_file_number_array:
            ...
            if parameters["input_file_format"] =="h5":
                parameters, complete_file_location = create_complete_file_location_view_roots_or_txt_script.create_complete_file_location_view_roots_or_txt(self.parameters, True, iteratable_number)   
    ...
    def plot_inputted_data(self, parameters, extra_plot_parameters):
        ...
        if parameters["input_file_format"] =="h5":
            parameters, complete_file_location = create_complete_file_location_view_roots_or_txt_script.create_complete_file_location_view_roots_or_txt(self.parameters, True, parameters["input_file_iteratable_file_number_start"])   
            y_values = self.get_single_intensity_spectrum_h5_galaxies(parameters, complete_file_location)
            x_values = [x for x in range(len(y_values))]
            plots.append([x_values, y_values, "Channel", "Counts" ])
            parameters, complete_file_location = create_complete_file_location_view_roots_or_txt_script.create_complete_file_location_view_roots_or_txt(self.parameters, True, parameters["input_file_iteratable_file_number_end"])   
            y_values = self.get_single_intensity_spectrum_h5_galaxies(parameters, complete_file_location)
            x_values = [x for x in range(len(y_values))]
            plots.append([x_values, y_values, "Channel", "Counts" ])
