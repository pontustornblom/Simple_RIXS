...
class MainWindow(QMainWindow):
    ...
    def initUI(self):      
        ...
        self.setWindowTitle("Simple RIXS Make treated Species RIXS")
        self.show()
    ...
    def update_dictionary_array(self, key, array_index, item):
        ...
        if key == "energy_per_channel_polynomial_coefficients_array":
            ...
            item_value = item.text() if hasattr(item, 'text') else item
            if self.validate_input_for_array(key, array_index, item):
                self.parameters[key][array_index] = item_value
            elif key == "input_complete_file_name_array":
                self.parameters[key][array_index] = item_value
    ...
    def validate_input_for_array(self, key, array_index, item):
        ...
        if item_value != self.parameters[key][array_index]:
            if item_value == "":
                QMessageBox.warning(
                ...
                try:
                    if item_value.startswith("0"):
                        float(iteratable_number_to_float_script.iteratable_number_to_float(item_value))
                    else:
                        float(item_value)
                    return True
                except ValueError:
    ...
    def validate_input(self, item, key):
        ...
        if item_value != self.parameters[key]:
            self.update_dictionary(key, item_value)
            if item_value == "":
                QMessageBox.warning(
                    self, "Invalid Input", "Do not leave blank"
                )
    ...
    def get_single_incoming_energy_spectrum_h5_veritas(self, parameters, iteratable_number):
        ...
        root_location = f"acq{iteratable_number}/startmetadata/beamline_energy/position"

        raw_data_list = []
        with h5py.File(complete_file_location, 'r') as file:
            try:
                dataset = file[root_location]

                # Check if the dataset is scalar or array
                if dataset.shape == ():  # Scalar dataset
                    data_value = dataset[()]
                    print(f"Dataset '{root_location}' is scalar: {data_value}")
                    raw_data_list.append(data_value)
                else:  # Array dataset
                    data_array = dataset[:]
                    print(f"Dataset '{root_location}' is array: {data_array}")
                    raw_data_list.extend(data_array)

                print("Data successfully added.")

            except KeyError:
                print(f"Dataset '{root_location}' does not exist in the HDF5 file.")

        print(raw_data_list)

        # Ensure each item has a second element if they're arrays/lists
        second_elements_array = []
        for item in raw_data_list:
            if isinstance(item, (list, tuple, np.ndarray)) and len(item) > 1:
                second_elements_array.append(item[1])
            else:
                second_elements_array.append(item)
